...
public abstract class RedisPERCache<T> {
	...
	public T perFetch(String key, double beta) {
	...
	private RedisPERData<T> findRedisCache(String key, TypeReference<RedisPERData<T>> typeReference) {
	...
	private T saveRedisPerCache(String key) {
		// ========= DB 조회 시간 ============== //
	...
	protected abstract TypeReference<RedisPERData<T>> getTypeReference();
	protected abstract RedisCacheInfo getCacheInfo(String key);
